//
// Created by Will Zhang on 2021/1/17.
//

#ifndef PITWINS_SERVO_H
#define PITWINS_SERVO_H
...
public:
    ...
    static void myPwmWrite(struct wiringPiNodeStruct *node, int pin, int value) {
        ...
        if (value >= 4096)
            getInstance()->pca9685FullOn(fd, ipin, 1);
        else if (value > 0)
            getInstance()->pca9685PWMWrite(fd, ipin, 0, value);    // (Deactivates full-on and off by itself)
        else
            getInstance()->pca9685FullOff(fd, ipin, 1);
    }
    ...
    static int myOffRead(struct wiringPiNodeStruct *node, int pin) {
        ...
        int off;
        getInstance()->pca9685PWMRead(fd, ipin, 0, &off);
    ...
    static int myOnRead(struct wiringPiNodeStruct *node, int pin) {
        ...
        int on;
        getInstance()->pca9685PWMRead(fd, ipin, &on, 0);

        return on;
    }
};

#endif //PITWINS_SERVO_H
